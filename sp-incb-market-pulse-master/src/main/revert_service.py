...
def revert_cron_operation(revert_data: Dict, entity_id: int) -> Dict:
    """
    Revert a cron job operation
    ...
    if action == 'delete':
        # Original action was CREATE → revert by deleting
        cron_service.delete_job(revert_data['job_id'])
        return {"message": f"Reverted: Deleted cron job ID {revert_data['job_id']}"}

    elif action == 'restore':
        # Original action was DELETE or UPDATE → revert by restoring
        job_data = revert_data.get('job_data', {})
        job_id = job_data.get('id', entity_id)

        # Check if job exists (UPDATE case) or needs recreation (DELETE case)
        existing_job = cron_service.get_job_by_id(job_id)
        ...
        if existing_job:
            ...
            cron_service.update_job(
                ...
                is_active=job_data.get('is_active')
            )
            return {"message": f"Reverted: Restored cron job {job_data.get('name')} to previous state"}
        else:
            # DELETE case: recreate the job
            jobs = cron_service.get_all_jobs()
            jobs.append(job_data)
            cron_service.save_jobs(jobs)
            # Re-schedule if active
            if job_data.get('is_active'):
                try:
                    from apscheduler.triggers.cron import CronTrigger
                    trigger = CronTrigger.from_crontab(job_data['schedule'], timezone=cron_service.TIMEZONE)
                    cron_service.scheduler.add_job(
                        func=cron_service.run_automation_task,
                        trigger=trigger,
                        args=[job_id, job_data['name']],
                        id=f"cron_job_{job_id}",
                        replace_existing=True
                    )
                except Exception as e:
                    logger.warning(f"Could not re-schedule restored job: {e}")
            return {"message": f"Reverted: Restored deleted cron job {job_data.get('name')}"}
...
def apply_revert(log_id: int, reverted_by: str = "admin") -> Dict:
    ...
    if module == 'rules':
        ...
        operation_result = revert_cron_operation(revert_data, entity_id)
